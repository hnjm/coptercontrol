...
//OpenSPI(SPI_FOSC_16, MODE_00, SMPEND );
            ...
            LATCbits.LATC7 = 0; // SDO output
            LATBbits.LATB1 = 0; // SCK output
            
            TRISCbits.TRISC7 = 0; // SDO clear
            TRISBbits.TRISB1 = 0; // SCK (clear in Master Mode; set in Slave Mode)
            TRISAbits.TRISA5 = 1;
            ...
            unsigned int dut = 255;
            CCP1CONbits.DC1B0 =(dut& 0x01); dut>>=1; //Least Significant bit
            CCP1CONbits.DC1B1 =(dut& 0x01); dut>>=1; // 2nd Least Significant bit
            CCPR1L = dut;

            // Set the PWM duty cycle by writing to the CCPRxL register and CCPxCON<5:4> bits.
            unsigned int dut = 255;
            CCP2CONbits.DC2B0 =(dut& 0x01); dut>>=1; //Least Significant bit
            CCP2CONbits.DC2B1 =(dut& 0x01); dut>>=1; // 2nd Least Significant bit
            CCPR2L = dut;
