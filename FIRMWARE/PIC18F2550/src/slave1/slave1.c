...
{   
    ...
    {   
        ...
        switch(ReceivedDataBuffer[0])				//Look at the data the host sent, to see what kind of application specific command it sent.
        {
            ...
            case 0x54:
                c = (ReceivedDataBuffer[1]);

                // Set the PWM duty cycle by writing to the CCPRxL register and CCPxCON<5:4> bits.
                CCP1CONbits.DC1B0 =(c& 0x01); c>>=1; //Least Significant bit
                CCP1CONbits.DC1B1 =(c& 0x01); c>>=1; // 2nd Least Significant bit
                CCPR1L = c;

                c = (ReceivedDataBuffer[2]);
                // Set the PWM duty cycle by writing to the CCPRxL register and CCPxCON<5:4> bits.
                CCP2CONbits.DC2B0 =(c& 0x01); c>>=1; //Least Significant bit
                CCP2CONbits.DC2B1 =(c& 0x01); c>>=1; // 2nd Least Significant bit
                CCPR2L = c;
